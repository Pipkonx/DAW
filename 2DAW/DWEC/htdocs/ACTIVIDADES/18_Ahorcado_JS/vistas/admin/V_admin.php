...
<?php
    ...
    // Simplified check for admin access without sessions
    $login_user = $_GET['login'] ?? '';
    $is_admin = $_GET['admin'] ?? 0; // Assuming 'admin' parameter indicates admin status

    if (empty($login_user) || $is_admin != 1) {
        $error_message = 'Acceso denegado. Solo administradores.';
    }
...
<body>
    ...
    <div class="flex-container">
        <a href="../juego/V_configurar.php?login=<?= $login_user ?>" class="base-button">Volver a Configurar Juego</a>
        <p>Usuario: <?= $login_user ?></p>
