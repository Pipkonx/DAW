...
@section('cuerpo')
  ...
  <form action="{{ $formActionUrl }}" method="POST" enctype="multipart/form-data">
    ...
    <a href="{{ dirname($_SERVER['SCRIPT_NAME']) }}/operario/tareas" class="btn btn-cancel">Cancelar</a>
  </form>

  <!-- Hidden Delete Forms -->
  @if(!empty($ficherosResumen))
     <form id="form-delete-resumen" action="{{ route('operario.tareas.eliminarFichero') }}" method="POST" style="display:none;">
         @csrf
  ...
  @if(!empty($fotos))
    @foreach($fotos as $foto)
        <form id="form-delete-foto-{{ md5($foto) }}" action="{{ route('operario.tareas.eliminarFichero') }}" method="POST" style="display:none;">
            @csrf
            <input type="hidden" name="id" value="{{ $id }}">
