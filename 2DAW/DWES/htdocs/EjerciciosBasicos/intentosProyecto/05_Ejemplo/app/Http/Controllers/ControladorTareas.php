...
class ControladorTareas extends Controller
{
    ...
    public function crear()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Validar datos
            $this->filtrar();
            if (!empty(Funciones::$errores)) return view('tareas/alta_edicion', $_POST);
    ...
    public function confirmarEliminar($id)
    {
        ...
        $tarea['id'] = (int)$id;
        return view('tareas/confirmar_eliminacion', $tarea);
    }
    ...
    public function detalle($id)
    {
        ...
        if (!$t) {
            ...
            $totalPaginas = 1;
            try {
                $tareas = $m->listar(self::TAREASXPAGINA, $paginaActual);
                $totalElementos = $m->contar();
                $totalPaginas = (int) max(1, ceil($totalElementos / self::TAREASXPAGINA));
            } catch (\Throwable $e2) {
                $tareas = [];
            }
