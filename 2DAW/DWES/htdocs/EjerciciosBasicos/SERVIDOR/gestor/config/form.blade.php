<div class="form-container">
    <form action="{{ $formActionUrl }}" method="POST" enctype="multipart/form-data">
        ...
        @endif
        <span class="radio-group-item"><input type="radio" id="estadoB" name="estadoTarea" value="B" {{ (($_POST['estadoTarea'] ?? ($estadoTarea ?? '')) == "B") ? "checked" : "" }} {{ $disableOperatorFields ? 'disabled' : '' }}> <label for="estadoB">Esperando ser aprobada</label></span>
        <span class="radio-group-item"><input type="radio" id="estadoP" name="estadoTarea" value="P" {{ (($_POST['estadoTarea'] ?? ($estadoTarea ?? '')) == "P") ? "checked" : "" }} {{ $disableOperatorFields ? 'disabled' : '' }}> <label for="estadoP">Pendiente</label></span>
        <span class="radio-group-item"><input type="radio" id="estadoR" name="estadoTarea" value="R" {{ (($_POST['estadoTarea'] ?? ($estadoTarea ?? '')) == "R") ? "checked" : "" }} {{ $disableOperatorFields ? 'disabled' : '' }}> <label for="estadoR">Realizada</label></span>
        <span class="radio-group-item"><input type="radio" id="estadoC" name="estadoTarea" value="C" {{ (($_POST['estadoTarea'] ?? ($estadoTarea ?? '')) == "C") ? "checked" : "" }} {{ $disableOperatorFields ? 'disabled' : '' }}> <label for="estadoC">Cancelada</label></span>
        ...
        @if($disableOperatorFields)
            <input type="hidden" name="fechaRealizacion"
                value="{{ $_POST['fechaRealizacion'] ?? ($fechaRealizacion ?? '') }}">
        @endif
        ...
        <textarea id="anotacionesAnteriores" name="anotacionesAnteriores"
            readonly>{{ htmlspecialchars($_POST['anotacionesAnteriores'] ?? ($anotacionesAnteriores ?? '')) }}</textarea><br><br>
        ...
        @if(!empty($ficherosResumen))
            <div class="file-item">
                <a href="{{ url('evidencias/' . $id . '/' . $ficherosResumen[0]) }}"
                    target="_blank">{{ $ficherosResumen[0] }}</a>
                <button type="submit" form="form-delete-resumen-admin" class="btn-danger"
                    style="font-size: 0.8em; padding: 2px 5px;">Eliminar</button>
            </div>
        @else
        ...
        @if(!empty($fotos))
            @foreach($fotos as $foto)
                <div class="file-item">
                    ...
                    <button type="submit" form="form-delete-foto-admin-{{ md5($foto) }}" class="btn-danger"
                        style="font-size: 0.8em; padding: 2px 5px;">Eliminar</button>
                </div>
            @endforeach
        @endif
    ...
    @if(isset($id) && !empty($ficherosResumen))
        <form id="form-delete-resumen-admin" action="{{ url('admin/tareas/eliminar-fichero') }}" method="POST"
            style="display:none;">
            @csrf
    ...
    @if(isset($id) && !empty($fotos))
        @foreach($fotos as $foto)
            <form id="form-delete-foto-admin-{{ md5($foto) }}" action="{{ url('admin/tareas/eliminar-fichero') }}" method="POST"
                style="display:none;">
                @csrf
                <input type="hidden" name="id" value="{{ $id }}">
