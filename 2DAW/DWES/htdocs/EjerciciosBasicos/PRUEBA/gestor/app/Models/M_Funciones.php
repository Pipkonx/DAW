...
class M_Funciones
{
    ...
    /**
     * Valida los datos de un usuario (nombre, contraseña, rol).
     *
     * @param array $datos Array asociativo con los datos del usuario.
     * @param bool $esEdicion Indica si la validación es para una edición (la contraseña puede ser opcional).
     * @return array Array asociativo con los errores encontrados, vacío si no hay errores.
     */
    public static function validarUsuario(array $datos, bool $esEdicion = false): array
    {
        $errores = [];
        ...
        // Validar rol
        $rolesValidos = ['admin', 'operario'];
        if (empty($datos['rol'])) {
            $errores['rol'] = 'El rol es obligatorio.';
        } elseif (!in_array($datos['rol'], $rolesValidos)) {
